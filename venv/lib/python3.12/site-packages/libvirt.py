...
class virNetwork(object):
    ...
    def metadata(self, type, uri, flags=0):
        ...
        configuration will be queried. """
        ret = libvirtmod.virNetworkGetMetadata(self._o, type, uri, flags)
        if ret is None:
            raise libvirtError('virNetworkGetMetadata() failed')
        return ret
    ...
    def setMetadata(self, type, metadata, key, uri, flags=0):
        ...
        or both will be modified. """
        ret = libvirtmod.virNetworkSetMetadata(self._o, type, metadata, key, uri, flags)
        if ret == -1:
            raise libvirtError('virNetworkSetMetadata() failed')
        return ret
...
VIR_MIGRATE_PARAM_COMPRESSION_ZLIB_LEVEL = "compression.zlib.level"
VIR_MIGRATE_PARAM_COMPRESSION_ZSTD_LEVEL = "compression.zstd.level"
